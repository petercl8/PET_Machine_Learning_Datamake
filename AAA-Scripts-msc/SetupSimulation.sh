...
##  - GateOutputFilesDirectory - The directory the root files will be saved.
...
then
	...
	echo "Usage: ScannerType GateOutputFilesDirectory Activity(Par/Filename) Attenuation(Par/Filename)"
	exit 1
fi

# Parameters
ScannerType=$1
GateOutputFilesDirectory=$2
ACTIVITY=$3
ATTENUATION=$4


## Get the scanner files into GATESubMacros directory.
./SubScripts/PrepareScannerFiles.sh $ScannerType $GateOutputFilesDirectory
if [ $? -ne 0 ]; then
...
#P -a is a flag used to pass parameters to Gate macro
Gate GATESubMacros/SetupDmap.mac -a [AttenuationFilename,$AttenuationFilename][GateOutputFilesDirectory,$GateOutputFilesDirectory]
