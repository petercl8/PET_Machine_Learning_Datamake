...
#	3:  CoincidenceData[prompt_sinogram]				- Coincidence data sinogram filename
#	4:  delayed_sinogram								- Delayed coincidence data singoram filename
#	5:  NormalisationSinogram[normalization_sinogram]				- Normalisation sinogram filename
...
if [[ $# == 7 || $# == 10 ]]; then
	...
	prompt_sinogram=$3
	delayed_sinogram=$4
	normalization_sinogram=$5
	atnimg=$6
	AttenIsGATE=$7
else
	echo "Usage: ${0} OutputMultiplicativeFactorsFilename OutputAdditiveSinogramFilename MeasuredData[prompt_sinogram] delayed_sinogram NormalisationSinogram[NormSino] AttenuationImage[atnimg] AttenuationIsGateOutput[AttenIsGATE] Optional:[num_scat_iters scatter_recon_num_subiterations scatter_recon_num_subsets]"
...
echo "    CoincidenceData[prompt_sinogram] = ${prompt_sinogram}"
echo "    delayed_sinogram = ${delayed_sinogram}"
echo "    NormalisationSinogram[normalization_sinogram] = ${normalization_sinogram}"
...
scatter_pardir="$STIR_install_dir/share/doc/STIR-6.0/examples/samples/scatter_estimation_par_files"
scatter_par="${scatter_pardir}/scatter_estimation.par"

## Estimate the randoms from delays
echo "====="
echo "Computing contribution due to randoms from delayed_sinogram"
sh ${SGCPATH}/DataCorrectionsComputation/EstimateRandomsFromDelayed.sh ${randoms3d} ${delayed_sinogram}
...
## Input data
export prompt_sinogram atnimg normalization_sinogram acf3d randoms3d scatter_pardir
...
calculate_attenuation_coefficients --ACF ${acf3d} ${atnimg} ${prompt_sinogram}
...
stir_math -s --mult ${OutputMultiplicativeFactorsFilename} ${normalization_sinogram} ${acf3d}
