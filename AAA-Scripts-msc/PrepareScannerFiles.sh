...
if [ $# -ne 2 ]; then
  echo "Usage:"$0 "ScannerType GateOutputFilesDirectory" 1>&2
  exit 1
fi
...
if [ ! -d $GateOutputFilesDirectory ]; then  # Does the output directory exist?
...
if [ $ScannerType = "D690" ]; then
	...
	cp -vp $D690_DIR/digitiser_D690.mac GATESubMacros/digitiser.mac #P Copies the first (source) file to the second. -v = verbose, p = preserves original file attributs (modification time, access time, and mode/permissions)
