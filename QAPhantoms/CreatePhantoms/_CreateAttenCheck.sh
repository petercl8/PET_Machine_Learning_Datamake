...
####
# Script for producing a QA phantom to make sure that the attenuation map is being properly aligned in reconstruction.
####
...
## XCAT Parameters
## ---------------
XCAT_phantomPath=${Path}XCAT_phantom/male_pt141_torso_D690.hv # Path to a full-size XCAT phantom (will be used to set the activity of the NEMA phantom so that the activity/slice is the same as for an XCAT phantom)
...
generate_image ${Path}ParFiles/attenCheck_act.par
generate_image ${Path}ParFiles/attenCheck_atn.par

stir_math --including-first --times-scalar 1 ${PhantomName}_act.hv attenCheck_act.hv
stir_math --including-first --times-scalar $WaterLAC ${PhantomName}_atn.hv attenCheck_atn.hv # Set background attenuation
