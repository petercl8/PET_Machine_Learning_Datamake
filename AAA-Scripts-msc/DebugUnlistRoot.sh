...
# - `sh ${SGCPATH}/VoxelisedSimulation/DebuggingScripts/Debug_UnlistRoot.sh ${SGCPATH}/VoxelisedSimulation/Output Sim_1 Coincidences`
...
UnlistingDirectory="${GateOutputFilesDirectory}/Unlisted/${EventDataType}/Debug"
...
if [ $# -lt 3 ]; then
  echo "Usage: $0 GateOutputFilesDirectory ROOT_FILENAME_PREFIX EventDataType [ AcceptanceProb(Default:1) [LowerEngeryThreshold(Default:0) UpperEngeryThreshold(Default:1000) [NumEventsToStore(Default=-1)]]]" 1>&2
  exit 1
fi
if [ $# -lt 6 ]; then
...
if [ ! -d $UnlistingDirectory ]; then
	...
	mkdir -p $UnlistingDirectory  ## Made from respect of $GateOutputFilesDirectory
fi
...
echo "Unlisting ${GateOutputFilesDirectory}/${ROOT_FILENAME}.root"
...
for EventType in Trues Scattered Randoms RandomScattered; do
	...
	LM_TO_PROJDATA_PAR_PATH=$GateOutputFilesDirectory/lm_to_projdata_${ROOT_FILENAME}.par
	cp  UnlistingTemplates/lm_to_projdata_template.par ${LM_TO_PROJDATA_PAR_PATH}
	sed -i.bak "s|{ROOT_FILENAME}|$GateOutputFilesDirectory/${ROOT_FILENAME}|g" ${LM_TO_PROJDATA_PAR_PATH}
	...
	ROOT_FILENAME_PATH="${GateOutputFilesDirectory}/${ROOT_FILENAME}.hroot"
