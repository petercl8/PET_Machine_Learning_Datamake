...
ScannerType="D690"
QT=1  # Tell RunGATEandUnlist.sh to use QT visualisation


## For "CheckGeometry.mac" these arguments have no effect, used to satisfy "RunGATEandUnlist.sh" arguments
GateOutputFilesDirectory=Output  # There is no file output from GATE
SectionNum=1  # No parallel processing required as no output
...
./RunGATEandUnlist.sh $GATEMainMacro root_filename $ActivityFilename $AttenuationFilename\
			$GateOutputFilesDirectory $SectionNum $StartTime $EndTime $QT
