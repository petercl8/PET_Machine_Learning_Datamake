...
####
# Script for producing a Pinwheel phantom. Activity is scaled so that the activity/cm (axial direction) for this phantom equals the activity/cm for a XCAT phantom.
####
...
## XCAT Parameters
## ---------------
XCAT_phantomPath=${Path}XCAT_phantom/male_pt141_torso_D690.hv # Path to a full-size XCAT phantom or XCAT phantom section of FOV (will be used to set the activity of the NEMA phantom so that the activity/slice is the same as the XCAT phantom)
XCAT_phantomHeight=153.69 # Height of the XCAT section (in mm). This is necesssary to calculate the activity/mm. For the D690, the height of each section is 153.69. the activity/mm.
...
generate_image ${Path}ParFiles/pinwheel_basicShape_30mm.par

stir_math --accumulate --including-first --max-threshold 2 BasicShape.hv # The center of the pinwheel, which will have a higher activity, is thresholded to $MaxActivity
stir_math --accumulate --including-first --min-threshold 1 BasicShape.hv # The whole phantom is thresholded to $BackgroundActivity (which is around the pinwheel)

# Create Activity Map
stir_math --including-first --add-scalar -1 ${PhantomName}_act.hv BasicShape.hv # The whole phantom activity is decreased by $BackgroundActivity. Now the background has activity = -1, the pinwheel =1, and the center = 2

# Create Attenuation Map
stir_math --including-first --add-scalar -1 ${PhantomName}_atn.hv BasicShape.hv
stir_math --including-first --accumulate --times-scalar $WaterLAC ${PhantomName}_atn.hv
...
if [ $Mask = 0 ]; then
    ...
    # Calculate activity/cm for current pinwheel phantom
...
medcon -f ${PhantomName}_atn.hv -c nifti
medcon -f ${PhantomName}_act.hv -c nifti
